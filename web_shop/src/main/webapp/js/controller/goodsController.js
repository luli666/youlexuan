...
app.controller('goodsController' ,function($scope,$controller,$location,typeTemplateService ,itemCatService,uploadService ,goodsService){	
	...
	$scope.entity={goods:{},goodsDesc:{itemImages:[],specificationItems:[]}};//定义页面的实体结构
	//添加图片的列表
	...
	$scope.$watch("entity.goods.typeTemplateId",function(newValue,oldValue){
        ...
        //查询规格列表
        typeTemplateService.findBySpecList(newValue).success(
            function(response){
    ...
    $scope.$watch('entity.goods.category3Id', function(newValue, oldValue) {
        itemCatService.findOne(newValue).success(
            function(response){
                $scope.entity.goods.typeTemplateId = response.itemCat.typeId; //更新模板ID
            }
        );
    });
});	
